...
inOut.forEach((i)=>{i.addEventListener("click",(e)=>{
...
async function play(e) {
    ...
    p[colour[k]].filter(j => { return ((position[colour[k]][p[colour[k]].indexOf(j)] + d) < (pos.length - 1) && ((position[colour[k]][p[colour[k]].indexOf(j)] == 0 && d == 6) || (position[colour[k]][p[colour[k]].indexOf(j)] != 0))) }).forEach(j => { j.removeEventListener('click', play, { once: true }) });
    ...
    if (d == 6 && position[colour[k]][j] == 0) {
        ...
        for (let l = 0; l < d; l++) {
            ...
            await sleep(200);
            setTimeout(()=>{time++},200);
            // while(time<l){setTimeout(()=>{time++});console.log(time);}
            // beforeSetPos(e.target, colour[k], position[colour[k]][j], j);

        }
        // position[colour[k]][j]+=d;
        // beforeSetPos(e.target, colour[k], position[colour[k]][j], j)
...
async function startGame() {
    ...
    // p[colour[k]].forEach(j => { j.style.border = '2px solid blue' });
    p[colour[k]].filter(j => { return (flag[colour[k]]==0 && !(d==6 && countOfSix==3) && (position[colour[k]][p[colour[k]].indexOf(j)] + d) <= (pos.length -1) && ((position[colour[k]][p[colour[k]].indexOf(j)] == 0 && d == 6) || (position[colour[k]][p[colour[k]].indexOf(j)] != 0))) }).forEach(j => { j.addEventListener('click', play, { once: true });j.style.setProperty(`z-index`,`${z}`) });

    // if the player has no move
    if (p[colour[k]].filter(j => { return (flag[colour[k]]==0 && !(d==6 && countOfSix==3) &&  (position[colour[k]][p[colour[k]].indexOf(j)] + d) <= (pos.length - 1) && ((position[colour[k]][p[colour[k]].indexOf(j)] == 0 && d == 6) || (position[colour[k]][p[colour[k]].indexOf(j)] != 0))) }).length == 0) {
    ...
    if (position[colour[k]][j] != 1 && position[colour[k]][j] <= pos.length - 7)
        for (let i = 0; i < colour.length; i++) {
            ...
            for (l = 0; l < colour.length; l++) {
                if (flag[l]==1 || position[i][l]==0 ||position[i][l]==1 || position[colour[k]][j]-position[i][l]==0|| position[i][l]>pos.length - 7);
...
// position[0][0]=55;
// beforeSetPos(p[0][0], 0, position[0][0], 0);
